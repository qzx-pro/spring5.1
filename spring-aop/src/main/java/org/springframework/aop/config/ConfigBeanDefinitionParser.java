...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		// compositeDef一般就是类似名为aop:config的对象
	...
	private void parseAspect(Element aspectElement, ParserContext parserContext) {
		...
		try {
			...
			// 获取aspect标签下的declare-parents子标签，一般不存在
			...
			// 获取aspect的子标签，也就是pointcut和各种advise
			NodeList nodeList = aspectElement.getChildNodes();
			boolean adviceFoundAlready = false;
			for (int i = 0; i < nodeList.getLength(); i++) {
				Node node = nodeList.item(i);
				// 对于advice节点进行处理，before,after,around,afterThrowing,afterReturning
				if (isAdviceNode(node, parserContext)) {
					if (!adviceFoundAlready) {
						...
						// 将切面aspect加入beanReferences集合中
						beanReferences.add(new RuntimeBeanReference(aspectName));
					}
					// 解析advice节点，并注册进bean工厂中
			...
			// 解析aop:pointcut标签，设置expression表达式所对应的值
	...
	private AbstractBeanDefinition parseAdvice(
		...
		try {
			...
			// 解析advice的method属性，并封装为MethodLocatingFactoryBean对象,可以理解为一个method对象
			// 里面含有targetBeanName，methodName和method对象
			...
			// 解析point-cut，并结合上述的两个bean最终包装为AbstractAspectJAdvice通知对象
	...
	private AbstractBeanDefinition createAdviceDefinition(
		...
		// 首先根据adviceElement节点分析出是什么类型的Advice,这里是AspectJMethodBeforeAdvice类型
		...
		// 解析point-cut属性，主要是获取expression表达式
	...
	private AbstractBeanDefinition parsePointcut(Element pointcutElement, ParserContext parserContext) {
		...
		try {
			...
			if (StringUtils.hasText(pointcutBeanName)) {
				//getRegistry获取到的就是DefaultListableFactory对象
