...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
			...
			throws BeansException {
		// 将带有&前缀的beanName去掉&前缀
		String beanName = transformedBeanName(name);
		Object bean;

		// Eagerly check singleton cache for manually registered singletons.
		// 提前检测单例缓存中是否存在手动注册的单例对象，和循环依赖有关
		...
		if (sharedInstance != null && args == null) {
			...
			// 返回对象的实例，在实现了FactoryBean接口之后，调用getObject方法的入口就是此方法。
			// 这里是判断是否执行了getObject方法，如果执行了就获取bean对象
			...
			// 当对象是单例的时候会尝试解决循环依赖问题，如果是原型的，出现循环依赖直接抛出异常
			if (isPrototypeCurrentlyInCreation(beanName)) {
				throw new BeanCurrentlyInCreationException(beanName);
			}

			// Check if bean definition exists in this factory.
			// 获取父容器，单一的spring不存在父容器，在springMVC中才会有
			...
			try {
				// 根据当前bean的name获取bean的完整的描述信息，包括父beanDefiniton，在第一次处理的时候会放在缓存中
				...
				if (mbd.isSingleton()) {
					// 单例对象的实例化
					// 传递的lamda表达式在执行ObjectFactory接口中的getObject方法的时候才会调用
