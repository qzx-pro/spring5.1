...
public class InitDestroyAnnotationBeanPostProcessor
	...
	private LifecycleMetadata buildLifecycleMetadata(final Class<?> clazz) {
		...
		do {
			...
			// 需要注意一点的是CommonAnnotationBeanPostProcessor中设置了initAnnotationType和destroyAnnotationType
			// 这样才能在这里直接进行将@PostConstruct和@PreDestroy和Class对象对应上

			// 反射获取当前类中的所有方法并依次对其调用第二个参数的lamda表达式
			ReflectionUtils.doWithLocalMethods(targetClass, method -> {
				// 判断当前方法的注解包含initAnnotationType注解(@PostConstruct)
				if (this.initAnnotationType != null && method.isAnnotationPresent(this.initAnnotationType)) {
					// 如果有，把它封装成LifecycleElement对象，存储起来
					LifecycleElement element = new LifecycleElement(method);
					// 添加到存储所有initMethod的集合中
					currInitMethods.add(element);
					if (logger.isTraceEnabled()) {
						logger.trace("Found init method on class [" + clazz.getName() + "]: " + method);
					}
				}
				// 判断当前方法的注解包含destroyAnnotationType注解(@PreDestroy)
			...
			// 先获取的子类，然后获取父类的initMethod和destroyMethod方法
