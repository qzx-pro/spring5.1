...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			// 合并父类BeanDefinition，从而生成一个RootBeanDefinition(之前都是GenericBeanDefinition)
			// 每个Bean的创建需要的是一个MergedBeanDefinition，
			// 也就是需要基于原始BeanDefinition及其双亲BeanDefinition信息得到一个信息"合并"之后的BeanDefinition；
			// 判断是否有父beanDefiniton的依据是判断当前的beanDefinition是否有parentName属性
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
